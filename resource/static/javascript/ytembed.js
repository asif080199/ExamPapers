		...
		*/	
		listVideos : function(json,cfg){
			...
			if(json.error){
				...
				var ul = document.createElement('ula');
				ul.className = 'ytlist';
				...
				for (var i = 0; i < json.data.items.length; i++) {
					...
					var li = document.createElement('lia');			
					...
					img.setAttribute('id', 'youtubeImage');
					...
					if(this.cfg.layout == 'thumbnails'){
						...
						//this.cfg.layout = full
						li.innerHTML = '<table class = "table"><tr><td valign="top" rowspan="2"></td><td valign="top"><b>'+entry.title+'</b><br/><span>'+this.formatDescription(entry.description)+'</span></td><td valign="top" style="width: 150px" class="meta"><div>From: <a href="http://www.youtube.com/profile?user='+entry.uploader+'">'+entry.uploader+'</a></div><div>Views: '+entry.viewCount+'</div><div>'+this.formatRating(entry.rating,entry.ratingCount)+' ratings</div>Time: '+this.formatDuration(entry.duration)+'</td></tr></table>';
